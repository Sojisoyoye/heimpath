...
## Azure Teardown — Completed (2026-06-20)

Azure decommissioning is complete. All steps below have been executed.

| Step | Status | Notes |
|------|--------|-------|
| Remove azurerm resource blocks from Terraform | ✅ Done | PR #392 |
| Switch Terraform backend to local | ✅ Done | PR #392 |
| `terraform init -migrate-state` + `terraform apply` | ⏭️ Skipped | Azure for Students subscription (`8b21f152`) was disabled and read-only — `terraform destroy` was blocked. Azure auto-deletes all resources when a subscription is disabled. |
| Purge Key Vaults | ⏭️ Skipped | Auto-deleted with subscription |
| Delete `heimpathtfstate` storage account | ⏭️ Skipped | Auto-deleted with subscription |
| Remove `AZURE_CREDENTIALS` GitHub secret | ✅ Done | 2026-06-20 |
| Remove `infra/terraform/` directory | ✅ Done | PR #394 |

No Azure resources or credentials remain in the system.
